...
public class CastRemoteDisplayUnityExtension : ICastRemoteDisplayExtension {
  ...
  public void Activate() {
    displaySimulator.StartCoroutine(EndOfFrameCoroutine());
  ...
  /**
   * Coroutine to trigger at the end of every frame. Make sure all cameras have finished rendering.
   */
  private IEnumerator EndOfFrameCoroutine() {
    while(displaySimulator.gameObject.activeInHierarchy) {
      yield return new WaitForEndOfFrame();
      RenderRemoteDisplay();
    }
  }

  private void RenderRemoteDisplay() {
